function [fNoQP,fHDot,NA,tauModel,SIGMA_fH,SIGMA_NA,HessianMatrixQP2Feet,gradientQP2Feet,...
          ConstraintsMatrixQP2Feet,bVectorConstraintsQp2Feet] = ...
              ...
              balancingControl_v3(M_r,M_s,h_r,h_s,J_r,J_f,J_s,J_rDot_nu,J_fDot_nu_s,J_sDot_nu_s,w_H_lSole,w_H_rSole,w_p_CoM_r,...
...
% CONTROL # 3: linear and angular momentum of the whole system
...
% projectors of momenta and their derivatives
...
b_XDot_s_star = [zeros(3) zeros(3);
                 skew(w_v_CoM_s-w_v_CoM_t) zeros(3)];
          
b_XDot_r_star = [zeros(3) zeros(3);
                 skew(w_v_CoM_r-w_v_CoM_t) zeros(3)];
...
biasForces_t = b_XDot_r_star*H_r + b_XDot_s_star*w_R_s_bar*H_s + ...
               b_X_r_star*M_r(1,1)*g*[e3;zeros(3,1)] + b_X_s_star*M_s(1,1)*g*[e3;zeros(3,1)] + ...
               b_X_s_star*w_R_s_bar*transpose(J_s)/(J_s*invM_s*transpose(J_s))*(J_s*invM_s*h_s -J_sDot_nu_s);
...
% forces  multiplier              
AT = b_X_r_star*AR - b_X_s_star*w_R_s_bar*transpose(J_f) + ...
     b_X_s_star*w_R_s_bar*transpose(J_s)/(J_s*invM_s*transpose(J_s))*J_s*invM_s*transpose(J_f);  
